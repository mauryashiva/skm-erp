...
export default function ParametersPage() {
  ...
  React.useEffect(() => {
    async function fetchPincodeCount() {
      if (!activeDivision) return;
      try {
        const data = await api.get<PincodeRecord[]>('/parameters/pincodes', {
          activeOnly: 'false',
        });
        setPincodeCount(Array.isArray(data) ? data.length : 0);
      } catch (err) {
        setPincodeCount(0);
      } finally {
        setIsLoading(false);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      {/* Available Parameter Modules — ONLY REAL PINCODE FORM */}
      <div>
          ...
          >
            <div>
              <div className="flex items-center justify-between gap-2 mb-3">
                ...
                <span className="text-xs font-bold font-mono px-2.5 py-1 rounded-md bg-secondary text-muted-foreground">
                  {isLoading ? '...' : `${pincodeCount ?? 0} Records`}
                </span>
              </div>
